...
export async function getStudyRoomSession(roomId: string) {
  ...
  if (error) throw new Error(getErrorMessage("Could not load room session", error));
  return data;
}
...
export async function saveStudyRoomSession(roomId: string, state: Json) {
  ...
  if (error) throw new Error(getErrorMessage("Could not save room session", error));
  return data;
}
...
export async function deleteStudyRoomSession(roomId: string) {
  ...
  if (error) throw new Error(getErrorMessage("Could not clear room session", error));
}
